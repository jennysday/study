			...
			<form action="regUser" method="post" name="regUserForm" id="regUserForm" target="iframe">
				<table>
					...
					<tr>
						...
						<td class="left"><input type="hidden" name="user_user_id"class="box" value="${sessionScope.user.user_id}">${sessionScope.user.user_id}</td>
					</tr>
...
function regUser() {
	...
	console.log($("input[name=checked_id]").val());
	 ...
	 if($("input[name='checked_id']").val()=='n'){
       alert('사용자 아이디를 확인 후 중복 버튼을 눌러주세요.');
	...
	if(user_id_v.length > 5) {
		$('#result').text('사용자 아이디는 5자 미만으로 입력해주세요.');
		form.user_id.select();
		return;
	}
