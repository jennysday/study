...
<c:forEach var="i" items="${sessionScope.titleFunctionList}">
	...
	<c:if test = "${fn:contains(i, 'Titl1001')}">
		<c:out value="${i}" /> 
		<c:set var="ok" value="true"></c:set>
				...
				<div class="btn_box_reg">
					<div>
						<ul>
							...
							<li><a href="javascript:regTitlemst()" class="btn" id="crbtn">등록</a></li>
						</ul>
					</div>
				</div>
			</form>
...
$(document).ready(function(){
	...
	if (ok == false) {
		...
		if(c == 0) {
			$("#crbtn").attr('class','disabled');
			alert('사용 권한이 없습니다.');location.href='getTitleList';s
		} else {
