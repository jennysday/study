	...
	<div class="wrap">
		...
		<div class="search">
			<div class="search_txt">
				<form action="getTitleListView" method="post" name="searchTitleForm" id="searchTitleForm">
					<span>직무코드 : &nbsp;</span> 
					<input class="underLine" type="text" name="titdet_titmst_id" id="titdet_titmst_id" autocomplete="off" value="${titdet_titmst_id} 검색 결과입니다.">
		...
		<div class="search">
			<div class="search_txt">
				<form action="getTitleListView" method="get" name="searchTitleForm" id="searchTitleForm">
						<span>직무코드 : &nbsp;</span> 
					...
					<c:choose>
						...
						<c:otherwise>
							<c:forEach var="title" items="${result.searchList}">
								<tr id="mouse" onClick="location.href='searchTitleList?titdet_titmst_id=${title.titdet_titmst_id}&titdet_pgm_id=${title.titdet_pgm_id}'">
			...
			<div class="paging">
				<div>
					...
					<c:if test="${result.page >= result.startPage}">
						<a href="searchTitleList?titdet_titmst_id=${titdet_titmst_id}&page=${result.page - 1}">[이전]</a>
					</c:if>
						...
						<c:forEach var="pageNum" begin="${result.startPage}" end="${result.endPage}"> 
							<c:choose>
								<c:when test="${pageNum eq result.page}">
									<span>
										<a style="font-weight: bold; color: #1d84df !important;" href="searchTitleList?titdet_titmst_id=${titdet_titmst_id}&page=${pageNum}">${pageNum}</a>
									</span>
								</c:when>
								<c:otherwise>
									<a href="searchTitleList?titdet_titmst_id=${titdet_titmst_id}&page=${pageNum}">${pageNum}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						
						<c:if test="${result.page < result.endPage}">
							<a href="searchTitleList?titdet_titmst_id=${titdet_titmst_id}&page=${result.page + 1}">[다음]</a>
						</c:if>
					</c:if>
				</div>
			</div>
			...
			<form action="updateTitle" method="post" name="TitleForm" id="TitleForm" target="iframe">
				<table class="table_left">
					...
					<tr>
						...
						<td><input type="hidden" name="titdet_function" id="titdet_function" value="${result2.title.titdet_function}" readonly="readonly"> </td>
					</tr>
					...
					<tr>
						<th>선택 기능</th>
						<td>${result2.title.titdet_function}</td>
					</tr>
