			...
			<div class="paging">
				<div>
					...
					<c:if test="${result.page >= result.startPage}">
						<a href="searchTitleList2?titdet_titmst_id=${sessionScope.getUserForm.titmst_id}&page=${result.page - 1}">[이전]</a>
					</c:if>
						...
						<c:forEach var="pageNum" begin="${result.startPage}" end="${result.endPage}"> 
							<c:choose>
								<c:when test="${pageNum eq result.page}">
									<span>
										<a style="font-weight: bold; color: #1d84df !important;" href="getTitleListView?titdet_titmst_id=${sessionScope.getUserForm.titmst_id}&page=${pageNum}">${pageNum}</a>
									</span>
								</c:when>
								<c:otherwise>
									<a href="searchTitleList2?titdet_titmst_id=${sessionScope.getUserForm.titmst_id}&page=${pageNum}">${pageNum}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						
						<c:if test="${result.page < result.endPage}">
							<a href="searchTitleList2?titdet_titmst_id=${sessionScope.getUserForm.titmst_id}&page=${result.page + 1}">[다음]</a>
						</c:if>
					</c:if>
				</div>
			</div>
