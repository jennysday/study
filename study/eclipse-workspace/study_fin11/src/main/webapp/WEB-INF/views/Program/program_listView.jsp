	...
	<div class="wrap">
		...
		<div class="search">
			<div class="search_txt">
				<form action="searchPrgmListView" method="post" name="searchPrgmForm" id="searchPrgmForm">
						<span>프로그램명 : &nbsp;</span> 
		...
		<div class="section">
			...
			<div class="paging">
				...
				<c:if test="${result.page >= result.startPage}">
					<a href="getProgramListView?page=${result.page - 1}">[이전]</a>
				</c:if>
					...
					<c:forEach var="pageNum" begin="${result.startPage}" end="${result.endPage}"> 
						<c:choose>
							<c:when test="${pageNum eq result.page}">
								<span>
									<a style="font-weight: bold; color: #1d84df !important;" href="getProgramListView?page=${pageNum}">${pageNum}</a>
								</span>
							</c:when>
							<c:otherwise>
								<a href="getProgramListView?page=${pageNum}">${pageNum}</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					
					<c:if test="${result.page < result.endPage}">
						<a href="getProgramListView?page=${result.page + 1}">[다음]</a>
					</c:if>
				</c:if>
			</div>
		</div>
		...
		<div class="section">
			<form action="updateProgram" method="post" name="updateProgramForm" id="updateProgramForm" target="iframe">
				<table class="table_left">
					<tr>
						<th>프로그램ID</th>
						<td class="left"><c:out value="${result3.program.pgm_id}"></c:out></td>
						<td><input type="hidden" name="pgm_id" id="pgm_id" value="${result3.program.pgm_id}" autocomplete="off"></td>
					</tr>
					<tr>
						<th>그룹코드</th>
						<td class="left"><c:out value="${result3.program.pgm_group}"></c:out></td>
						<td><input type="hidden" name="pgm_group" id="pgm_group" value="${result3.program.pgm_group}" autocomplete="off"></td>
					</tr>
					<tr>
						<th><label for="pgm_name">프로그램명</label></th>
						<td class="left"><input type="text" name="pgm_name" id="pgm_name" value="${result3.program.pgm_name}" class="box2" autocomplete="off" maxlength='20'></td>
					</tr>
					...
					<tr>
						...
						<td class="left"><input type="text" name="pgm_status" id="pgm_status" class="underLine" autocomplete="off" maxlength='4' value="${result3.program.pgm_status}" maxlength='1' readonly class="underLine"></td>
					</tr>
					
					
					<tr>
						<th>최종사용자</th>
						<td class="left"><c:out value="${result3.program.pgm_user_id}"></c:out></td>
...
$(document).ready(function(){
	...
	$("select[name=team_upteam]").val('${result3.team.team_upteam}');
