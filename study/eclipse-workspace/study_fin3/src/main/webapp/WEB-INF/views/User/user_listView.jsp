	...
	<div class="wrap">
		...
		<div class="search">
			<div class="search_txt">
				<form action="searchUserListView" method="post" name="searchUserForm" id="searchUserForm">
						<span>사용자명 : &nbsp;</span> 
					...
					<c:choose>
						...
						<c:otherwise>
							<c:forEach var="user" items="${result.userList}">
								<tr id="mouse" onClick="location.href='getUserListView?user_id=${user.user_id}&page=${result.page}'">
			...
			<div class="paging">
				<div>
					...
					<c:if test="${result.page >= result.startPage}">
						<a href="getUserListView?page=${result.page - 1}">[이전]</a>
					</c:if>
						...
						<c:forEach var="pageNum" begin="${result.startPage}" end="${result.endPage}"> 
							<c:choose>
								<c:when test="${pageNum eq result.page}">
									<span>
										<a style="font-weight: bold; color: #1d84df !important;" href="getUserListView?page=${pageNum}">${pageNum}</a>
									</span>
								</c:when>
								<c:otherwise>
									<a href="getUserListView?page=${pageNum}">${pageNum}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						
						<c:if test="${result.page < result.endPage}">
							<a href="getUserListView?page=${result.page + 1}">[다음]</a>
						</c:if>
					</c:if>
				</div>
			</div>
