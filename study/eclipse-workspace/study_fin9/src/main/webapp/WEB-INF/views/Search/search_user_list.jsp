	...
	<div class="wrap">
		...
		<div class="search">
			<div class="search_txt">
				<form action="getUserList" method="post" name="searchUserForm" id="searchUserForm">
						<span>사용자명 : &nbsp;</span> 
						<select name="user_id" class="teamList">
						     <c:forEach var="result" items="${result3.userList}">
		...
		<div class="section user_section">
			<table id="gcTable" class="tablesorter">
				...
				<tbody>
					<c:choose>
						<c:when test="${fn:length(result2.searchList) eq 0}">
							<tr>
						...
						<c:otherwise>
							<c:forEach var="user" items="${result2.searchList}">
			...
			<div class="paging">
				<div>
					...
					<c:if test="${result2.page >= result2.startPage}">
						<a href="searchUserList?page=${result2.page - 1}">[이전]</a>
					</c:if>
						...
						<c:forEach var="pageNum" begin="${result2.startPage}" end="${result2.endPage}"> 
							<c:choose>
								<c:when test="${pageNum eq result2.page}">
									<span>
										<a style="font-weight: bold; color: #1d84df !important;" href="searchUserList?page=${pageNum}">${pageNum}</a>
									</span>
								</c:when>
								<c:otherwise>
									<a href="searchUserList?page=${pageNum}">${pageNum}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						
						<c:if test="${result2.page < result.endPage}">
							<a href="searchUserList?page=${result2.page + 1}">[다음]</a>
						</c:if>
					</c:if>
				</div>
			</div>
