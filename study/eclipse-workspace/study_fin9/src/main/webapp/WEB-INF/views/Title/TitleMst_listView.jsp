					...
					<c:choose>
						...
						<c:otherwise>
							<c:forEach var="title" items="${result.titleMstList}">
								<tr id="mouse" onClick="location.href='getTitleMstListView?titmst_id=${title.titmst_id}&page=${result.page}'">
			...
			<form action="updateMstTitle" method="post" name="TitleForm" id="TitleForm" target="iframe">
				<table class="table_left">
					...
					<tr>
						...
						<td class="left"><input type="text" name="titmst_name" id="titmst_name" value="${result2.title.titmst_name}" autocomplete="off" maxlength='20'></td>
					</tr>
	...
	function delTitle() {
		...
		if(${sessionScope.user.user_id != null}){
			...
			if(chk) {
				...
				var titmst_id = $("input[name=titmst_id]").val();
				console.log(titmst_id);
				
				$.ajax({
						
		            type: 'POST',
		            url: 'delMstTitle',
		            data: 'titmst_id=' + titmst_id,
		            dataType: 'text',
		            success: function(result) {
		            	console.log("성공")
