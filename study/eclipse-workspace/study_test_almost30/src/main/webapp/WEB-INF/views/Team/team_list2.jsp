...
<body>
	...
	<c:if test="${sessionScope.ok}">
	
	</c:if>
	<c:out value="${sessionScope.ok}"></c:out>
	
	<c:forEach var="i" items="${sessionScope.titleDetail}">
	  <c:out value="${i.titdet_titmst_id}"></c:out>
	  <c:out value="${i.titdet_pgm_id}"></c:out>
	  <c:out value="${i.titdet_function}"></c:out>
	</c:forEach>
