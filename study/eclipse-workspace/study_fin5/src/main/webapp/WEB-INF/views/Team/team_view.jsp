...
<c:forEach var="i" items="${sessionScope.titleFunctionList}">
	...
	<c:if test = "${fn:contains(i, 'Team1004')}">
		<c:out value="${i}" /> 
		<c:set var="ok" value="true"></c:set>
...
<c:out value="${ok}" /> 
	...
	<div class="wrap">
		...
		<c:if test="${empty result.team.team_id}">
			<div class="msg">
				<p>MSG :<span id="result">삭제가 완료되었습니다.</span></p>
			</div>
		</c:if>
		<c:if test="${not empty result.team.team_id}">
			<div class="msg">
