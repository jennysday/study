						...
						<c:otherwise>
							<c:forEach var="title" items="${result.titleList}">
								<tr id="mouse" onClick="location.href='getTitle?titdet_titmst_id=${title.titdet_titmst_id}&titdet_pgm_id=${title.titdet_pgm_id}'">
									...
									<td><fmt:formatDate value="${title.titmst_reg_date}" type="both" pattern="yyyy-MM-dd"/></td>
									<td><fmt:formatDate value="${title.titmst_update_date}" type="both" pattern="yyyy-MM-dd"/></td>
									<td><c:out value="${title.titmst_user_id}"></c:out></td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
