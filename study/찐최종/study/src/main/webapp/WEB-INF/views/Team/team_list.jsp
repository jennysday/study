...
<%-- 
	<c:forEach var="i" items="${sessionScope.titleDetail}">
	  <c:out value="${i.titdet_pgm_id}   ${i.titdet_function}"></c:out>
	</c:forEach>
...
<c:forEach var="i" items="${sessionScope.titleFunctionList}">
	...
	<c:if test = "${fn:contains(i, 'Team1002')}">
		<c:out value="${i}" /> 
		<c:set var="ok" value="true"></c:set>
	...
	<div class="wrap">
		...
		<div class="search">
			<div class="search_txt">
				<form action="searchTeamList" method="post" name="searchTeamForm" id="searchTeamForm">
						<span>부서명 : &nbsp;</span> 
						...
						<c:otherwise>
							<c:forEach var="team" items="${result.teamList}">
								<tr id="mouse" onClick="location.href='getTeamListView?team_id=${team.team_id}&page=${result.page}'">
									...
									<td><fmt:formatDate value="${team.team_reg_date}" type="both" pattern="yyyy-MM-dd"/></td>
									<td><fmt:formatDate value="${team.team_update_date}" type="both" pattern="yyyy-MM-dd"/></td>
									<td><c:out value="${team.team_user_id}"></c:out></td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
